...
const CLEAR_CAMERA_URL = process.env.CLEAR_CAMERA_URL;  
...
function Cameras() {
    ...
    useEffect(() => {
        ...
        if (currentEmail !== prevEmail || currentDiscord !== prevDiscord) {
            ...
            try {
                const response = await axios.get(`${API_SERVER_URL}/cameras?userId=${user_id}`);
                const rooms = response.data;

                for (const room of rooms) {
                    const src = room.src || "None";
                    await axios.get(`${CLEAR_CAMERA_URL}/clear_camera?src=${src}&user_id=${user_id}`);
                }
    ...
    useEffect(() => {
        ...
        axios.get(`${API_SERVER_URL}/cameras?userId=${user_id}`)  
            .then((response) => {
    ...
    const handleSave = () => {
        ...
        if (modalMode === "add") {
            axios.post(`${API_SERVER_URL}/cameras`, updatedCamera, {
            ...
            axios.get(`${CLEAR_CAMERA_URL}/clear_camera?src=${modalData.src}&user_id=${user_id}`);
            axios.put(`${API_SERVER_URL}/cameras/${modalData._id}`, updatedCamera, {
                headers: { 'Content-Type': 'application/json' },
            })
            .then((response) => {
    ...
    const handleDeleteCamera = (id) => {
        axios.delete(`${API_SERVER_URL}/cameras/${id}`)
        .then(() => {
        ...
        for (const room of rooms) {
            const src = room.src || "None";
            axios.get(`${CLEAR_CAMERA_URL}/clear_camera?src=${src}&user_id=${user_id}`);
        }
        window.location.reload();
    };
